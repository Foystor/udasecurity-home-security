...
public class SecurityService {
    ...
    /**
     * Check if the all sensors state are the same as the state provided
     * @param state the activation status we want to check
     * @return true if all sensors state are the same as the one provided, otherwise false
     */
    private boolean checkAllSensorsForState(boolean state) {
    ...
    private void catDetected(Boolean cat) {
        ...
        if(cat && getArmingStatus() == ArmingStatus.ARMED_HOME) {
            setAlarmStatus(AlarmStatus.ALARM);
        } else if (!cat && checkAllSensorsForState(false)){
            setAlarmStatus(AlarmStatus.NO_ALARM);
        }
